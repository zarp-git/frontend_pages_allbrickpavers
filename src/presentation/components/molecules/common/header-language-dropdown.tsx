...
export function HeaderLanguageDropdown({
  ...
  if (variant === "mobile") {
    return (
      <div
        className={cn(
          "flex items-center border border-gray-700 rounded-md px-3 py-2 bg-gray-900/50",
          className,
        )}
      >
        <RiGlobalLine className="h-5 w-5 mr-3 text-gray-400" />
        <select
          className="w-full bg-transparent text-white focus:outline-none"
          defaultValue="pt-BR"
        >
          <option value="pt-BR" className="bg-gray-900">
            PT
          </option>
        </select>
      </div>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center text-base font-normal text-white hover:text-primary">
          <RiGlobalLine className="h-4 w-4 mr-1" />
        </button>
      </DropdownMenuTrigger>
