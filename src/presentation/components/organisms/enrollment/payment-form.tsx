...
const PRODUCTS = [
  {
    ...
    description:
      "O passo a passo para analisar um FII e montar sua estratégia.",
...
const INPUT_BASE_CLASSES =
  "w-full h-12 px-3 py-2 bg-[#F5F5F5] rounded-lg border border-transparent focus:ring-2 focus:ring-[#0047FF] focus:outline-none transition-colors disabled:opacity-50 disabled:cursor-not-allowed";
...
const SecurityInfo: React.FC = () => (
  <div className="flex justify-center items-center gap-2.5 py-4">
    <RiShieldLine className="size-4 text-zinc-500" />
...
const SubscriptionTerms: React.FC = () => (
  <div className="text-center px-2">
    <p className="text-zinc-500 text-[10px] sm:text-xs font-normal leading-relaxed">
      Ao adquirir um curso no Ponte Américas, você concorda que sua assinatura
      se renovará automaticamente a cada ano para garantir acesso contínuo sem
      interrupções. Caso decida não renovar, você poderá cancelar a renovação
      automática a qualquer momento antes da data de renovação.
    </p>
  </div>
);
...
export const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const handleFormattedInput =
    (fieldName: keyof PaymentData, formatter: (value: string) => string) =>
