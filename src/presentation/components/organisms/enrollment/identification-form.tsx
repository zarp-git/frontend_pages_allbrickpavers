...
import { PasswordField } from "@/presentation/components/forms/password-field";
import { InternationalPhoneField } from "@/presentation/components/forms/international-phone-field";
...
const getFeedbackIcon = (type: "success" | "error") =>
  type === "success" ? (
    <RiCheckboxCircleLine className="w-5 h-5 text-green-600 shrink-0" />
  ) : (
    <RiShieldLine className="w-5 h-5 text-red-600 shrink-0" />
  );
...
const RetryIndicator: React.FC<{ retryCount: number }> = ({ retryCount }) => (
  <div className="mt-2 flex items-center gap-2">
    ...
    <span className="text-xs text-orange-600">Tentativa {retryCount}/3</span>
  </div>
);
  ...
  return (
    ...
    >
      ...
      <PrimaryButton
        type="submit"
        size="lg"
        className={`w-full h-12 px-8 py-4 rounded-lg text-white font-medium uppercase transition-all duration-200 ${getButtonClassName(
          isLoading,
          hookLoading,
          isValid,
        )}`}
        ...
        icon={
          !isFormLoading ? <RiArrowRightLine className="w-5 h-5" /> : undefined
        }
        iconPosition="right"
      >
      ...
      {feedbackMessage.type && (
        <div
          className={`mt-4 p-4 rounded-lg border transition-all duration-300 ${getFeedbackClassName(
            feedbackMessage.type,
          )}`}
        >
